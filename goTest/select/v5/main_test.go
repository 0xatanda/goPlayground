...
func TestRacer(t *testing.T) {
	...
	t.Run("Return an err if a server doesn't response within the specfied time", func(t *testing.T) {
		sever := makeDelay(25 * time.Millisecond)

		defer sever.Close()

		_, err := ConfigRacer(sever.URL, sever.URL, 20*time.Millisecond)

		if err == nil {
			t.Errorf("expected an err but didn't get one")
		}
	})
}
